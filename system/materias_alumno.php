...
<html lang="es">
                                      ...
                                      <hr>
                                        ...
                                        <tbody id="materias">
                                          <?php
                                            ...
                                            $resultado=$mysqli->query("SELECT cm.Id_Cursa_Materia,m.Titulo,pe.Fecha_Inicio,pe.Fecha_Fin,p.Nombres,p.Apellido_P,p.Apellido_M,cm.Grupo FROM cursa_materia cm INNER JOIN materia m ON m.Id_Materia=cm.Id_Materia INNER JOIN periodo pe ON pe.Id_Periodo=cm.Id_Periodo INNER JOIN profesor p ON p.Id_Profesor=cm.Id_Profesor WHERE Id_Alumno={$usuario} AND cm.Estado='1'")or die("Error en: ".$mysqli->error);
  ...
  <script>
    ...
    //comprobar
    $.post("../control/ajax.php?option=28",{permiso:"1"}).done(function(data){
      if(data=="success"){
    ...
    $("#salir").click(function(){
      $.post("../control/ajax.php?option=29",{terminar:1}).done(function(data){
        if(data=="success"){
