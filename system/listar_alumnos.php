  ...
  <section id="container" >
      ...
      *********************************************************************************************************************************************************** -->
      ...
      <section id="main-content">
          <section class="wrapper">
            <h3><i class="fa fa-angle-right"></i>Lista de Alumnos</h3>
            <!-- SORTABLE TO DO LIST -->
               <div class="row mt" id="actual">
                  <div class="col-md-12">
                      <div class="content-panel">
                          <table class="table table-striped table-advance table-hover">
                            <h4><i class="fa fa-angle-right"></i>Todos los alumnos</h4>
                            <hr>
                              <thead>
                              <tr>
                                  <th><i class="fa fa-bookmark"></i> Alumnos</th>
                              ...
                              <?php
                                $cont=0;
                                $resultado=$mysqli->query("SELECT Id_Alumno, Nombres, Apellido_P, Apellido_M, Curp FROM alumno")or die("Error en: ".$mysqli->error);
                                while($row=$resultado->fetch_array(MYSQLI_ASSOC)){
                                  ...
                                  <tr>
                                    ...
                                    <td>
                                        ...
                                        <a class="btn btn-primary btn-xs" href="modificar_alumno.php?id='.$row['Id_Alumno'].'"><i class="fa fa-pencil" ></i></a>
                                        <button class="btn btn-danger btn-xs delete" data="'.$row['Id_Alumno'].'" data-toggle="modal" href="#elimiModal" ><i class="fa fa-trash-o "></i></button>
                                    </td>
                                  </tr>';
                                  $cont++;
                                }
                      ...
                      <div class=" add-task-row">
                        <a class="btn btn-success btn-sm pull-left" href="alta_alumno.php">Alta Nuevo Alumno</a>
                      </div>
                      <?php
                        $cont2=0;
                        $resultado=$mysqli->query("SELECT a.Nombres, a.Apellido_P, a.Apellido_M, a.Sexo, a.Direccion, l.Localidad, m.Municipio, e.Estado, a.CP, a.Fecha_Nacimiento, a.Curp, a.Img FROM alumno a INNER JOIN localidad l INNER JOIN municipio m INNER JOIN estado e ON a.Id_Localidad=l.Id_Localidad AND l.Id_Municipio=m.Id_Municipio AND m.Id_Estado=e.Id_Estado WHERE a.Estado='1'")or die("Error en: ".$mysqli->error);
                        while($row=$resultado->fetch_array(MYSQLI_ASSOC)){
                          echo' <!-- Modal -->
                      ...
                      <div class="modal fade" id="elimiModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                        <div class="modal-dialog">
                          <div class="modal-content">
                            ...
                            <div class="modal-body">
                              ¿Está seguro que desea eliminar el alumno? No podrá deshacer los cambios.
                            </div>
    ...
    $(document).ready(function() {
      ...
      $('#eliminar').click(function(){
        ...
        $.ajax({
            type: "POST",
            url: "../control/ajax.php?option=40",
            data: dataString,
            success: function() {
