  ...
  <section id="container" >
      ...
      *********************************************************************************************************************************************************** -->
      ...
      <section id="main-content">
          <section class="wrapper">
            <h3><i class="fa fa-angle-right"></i>Lista de Tutores</h3>
            <!-- SORTABLE TO DO LIST -->
               <div class="row mt" id="actual">
                  <div class="col-md-12">
                      <div class="content-panel">
                          <table class="table table-striped table-advance table-hover">
                            <h4><i class="fa fa-angle-right"></i>Todos los Tutores</h4>
                            <hr>
                              <thead>
                              <tr>
                                  <th><i class="fa fa-bookmark"></i>Tutores</th>
                              ...
                              <?php
                                $cont=0;
                                $resultado=$mysqli->query("SELECT Id_Tutor, Nombres, Apellido_P, Apellido_M, CURP FROM tutor")or die("Error en: ".$mysqli->error);
                      ...
                      <div class=" add-task-row">
                        <a class="btn btn-success btn-sm pull-left" href="alta_tutor.php">Alta Nuevo Tutor</a>
                      </div>
                      <?php
                        $cont2=0;
                        $resultado=$mysqli->query("SELECT t.Nombres, t.Apellido_P, t.Apellido_M, t.Sexo, t.Direccion, l.Localidad, m.Municipio, e.Estado, t.CP, t.Fecha_Nacimiento, t.CURP, t.Email, t.Telefono, t.Img FROM tutor t INNER JOIN localidad l INNER JOIN municipio m INNER JOIN estado e ON t.Id_Localidad=l.Id_Localidad AND l.Id_Municipio=m.Id_Municipio AND m.Id_Estado=e.Id_Estado WHERE t.Estado='1'")or die("Error en: ".$mysqli->error);
                        while($row=$resultado->fetch_array(MYSQLI_ASSOC)){
                          echo' <!-- Modal -->
                      ...
                      <div class="modal fade" id="elimiModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                        <div class="modal-dialog">
                          <div class="modal-content">
                            <div class="modal-header">
                              <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                              <h4 class="modal-title" id="myModalLabel">Eliminar Tutor</h4>
                            </div>
                            <div class="modal-body">
                              ¿Está seguro que desea eliminar el Tutor? No podrá deshacer los cambios.
                            </div>
    ...
    $(document).ready(function() {
      ...
      $('#eliminar').click(function(){
        ...
        $.ajax({
            type: "POST",
            url: "../control/ajax.php?option=43",
            data: dataString,
            success: function() {
